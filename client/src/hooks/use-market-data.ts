...
export function useMarketData(initialWatchlistItems: WatchlistItem[]) {
  ...
  const fetchLTPData = useCallback(async (itemsToFetch: WatchlistItem[]) => {
    ...
    if (groupedByExchange.NSE.length > 0) {
      exchangeTokenPayload.push({
        exchange: "NSE",
        tokens: groupedByExchange.NSE,
      });
    }
    if (groupedByExchange.BSE.length > 0) {
      exchangeTokenPayload.push({
        exchange: "BSE",
        tokens: groupedByExchange.BSE,
      });
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ message: "Failed to fetch LTP data" }));
      ...
      setTickerData((prevData) => {
        ...
        const currentWatchlistKeys = new Set(
          itemsToFetch.map((i) => `${i.exchange}:${i.token}`)
        );
  ...
  return { tickerData, isLoading, error, fetchLTPData };
}
