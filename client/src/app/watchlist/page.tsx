...
export default function WatchlistPage() {
  const {
    ...
    isLoaded: isWatchlistLoaded, // from localStorage
  } = useWatchlist();

  // Memoize watchlistItems to prevent unnecessary re-renders of useMarketData
  ...
  const { tickerData, isLoading: isMarketDataLoading, error: marketDataError } = useMarketData(watchlistItems);
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 p-4 md:p-8">
      <div className="max-w-3xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-semibold text-sky-400">My Watchlist</h1>
        ...
        {isMarketDataLoading && displayItems.length === 0 && (
            <div className="text-center py-4 text-slate-400">Fetching initial market data...</div>
        )}

        {displayItems.length === 0 && !isMarketDataLoading && (
          <div className="text-center py-10 text-slate-500">
            Your watchlist is empty. Click &quot;Add Symbol&quot; to get started.
          </div>
        )}
          ...
          <div className="bg-slate-800 shadow-xl rounded-lg overflow-hidden">
            {displayItems.map((item) => (
              <WatchlistItemDisplay
                ...
                ticker={item as any} // Cast as TickerData; ensure all fields are present
